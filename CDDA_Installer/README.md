...
> **폰트 주의**: 폰트는 `data/font/`에 복사만으로는 적용되지 않는다. 게임은
> `config/fonts.json`의 `typeface`/`gui_typeface`/`map_typeface`/`overmap_typeface`가
> 가리키는 폰트만 사용하므로, 추가한 폰트를 쓰려면 그 파일을 직접 편집해야 한다
> (게임 내 폰트 선택기 없음). 그래서 기본 폰트(Terminus/unifont 등)는 삭제 보호된다.
