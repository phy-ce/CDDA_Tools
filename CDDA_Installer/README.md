...
(게임 내 폰트 선택기 없음). 그래서 폰트 탭에는 **🅰 적용** 버튼이 있다:

1. 폰트를 추가(또는 기본 폰트 선택)하고 **🅰 적용** 클릭
2. 적용할 영역 선택 — 기본(게임 텍스트) / 메뉴(GUI) / 지도 / 전체지도
3. 도구가 `config/fonts.json`을 편집해 그 폰트를 **주 글꼴로 맨 앞에** 넣는다
   (기존 폰트는 폴백으로 유지, 이전 설정은 `fonts.json.bak`로 백업, 게임 재시작 시 반영)

`config/fonts.json`은 게임 첫 실행 시 `data/fontdata.json` 기본값으로 생성되며,
형식은 CDDA 공식 문서 `doc/user-guides/FONT_OPTIONS.md`를 따른다.
기본 폰트(Terminus/unifont 등)는 삭제 보호되어 실수로 지워 게임이 깨지는 일을 막는다.
